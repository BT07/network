...
module.exports = (function() {
	return{
		get_all_cert: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
				...
				return fabric_client.getUserContext('PCCE', true);
			}).then((user_from_store) => {
				if (user_from_store && user_from_store.isEnrolled()) {
					console.log('Successfully loaded PCCE from persistence');
					member_user = user_from_store;
				} else {
					throw new Error('Failed to get PCCE.... run VerifierUser.js');
				}
	...
	get_student: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // get the enrolled user from persistence, this user will sign all requests
		    return fabric_client.getUserContext('GU', true);
		}).then((user_from_store) => {
		    if (user_from_store && user_from_store.isEnrolled()) {
		        console.log('Successfully loaded GU from persistence');
		        member_user = user_from_store;
		    } else {
				throw new Error('Failed to get GU.... run UniversityUser.js');
		    }
